...
export default function Cart({ howManyProducts, cart, setCart, animation }) {
  const [open, setOpen] = useState(false);

  const number = useRef();

  useEffect(() => {
    number.current.classList.add("animate-spin");

    const timeoutId = setTimeout(() => {
      number.current.classList.remove("animate-spin");
    }, 1000);
  ...
  return (
    <>
      <li
        ...
        className={` mt-1 block align-middle font-sans text-white lg:inline-block`}
        ref={number}
      >
